...
class PunchViewModel(application: Application) : AndroidViewModel(application) {
    companion object {
        ...
        private const val KEY_BASS_FREQ = "bass_freq"
    ...
    /** 0..100: broad low-frequency reinforcement implemented by DynamicsProcessing. */
    var bassAmount by mutableFloatStateOf(0f)
        private set

    /** Center frequency of the bass reinforcement, independently selectable from Punch. */
    var bassFrequencyHz by mutableFloatStateOf(45f)
        private set

    /** Requested bass-centering mode. Android DynamicsProcessing has no channel-summing primitive. */
    var bassMono by mutableStateOf(true)
        private set

    /** 0..100: controlled harmonic emphasis implemented as extra post-EQ peaks. */
    ...
    fun loadSaved() {
        ...
        bassFrequencyHz = prefs.getFloat(KEY_BASS_FREQ, 45f).coerceIn(25f, 120f)
    ...
    fun save() {
        getApplication<Application>().getSharedPreferences(PREFS, Context.MODE_PRIVATE)
            ...
            .putFloat(KEY_BASS_FREQ, bassFrequencyHz)
