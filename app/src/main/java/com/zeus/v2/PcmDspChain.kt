...
class PcmDspChain(sampleRate: Int = 48000) {
    ...
    fun configure(sampleRate: Int, source: EqSettings) {
        ...
        if (newSr != sr) {
            ...
            bass.reset()
            var n = 0
            while (n < 6) { crossL[n].reset(); crossR[n].reset(); n++ }
            env.fill(0f)
            compGain[0] = 1f; compGain[1] = 1f; compGain[2] = 1f; compGain[3] = 1f
            limiterGain = 1f
        }
        settings = source
        bass.bassMono = source.bassMono
        bass.bassAmount = source.bassAmount.coerceIn(0f, 100f)
        bass.punchAmount = source.bassPunch.coerceIn(0f, 100f)
        bass.harmonicAmount = source.bassHarmonics.coerceIn(0f, 100f)
        // Legacy SubBoost remains useful as a gentle bass amount when the new control is zero.
        if (source.bassAmount == 0f && source.subBoost > 0f) bass.bassAmount = source.subBoost.coerceIn(0f, 100f)
        rebuildEq(source)
    ...
    fun process(buffer: FloatArray, offset: Int = 0, frames: Int = (buffer.size - offset) / 2) {
        ...
        // Bass must precede EQ so the EQ can shape the generated/controlled low end.
        bass.processStereo(buffer, sr, safeOffset, safeFrames)
    ...
    private fun configureDynamics(s: EqSettings) {
        ...
        preGain[0] = dbToLinear(s.compPreGainLow.coerceIn(-24f, 24f)); preGain[1] = dbToLinear(s.compPreGainLoMid.coerceIn(-24f, 24f)); preGain[2] = dbToLinear(s.compPreGainHiMid.coerceIn(-24f, 24f)); preGain[3] = dbToLinear(s.compPreGainHigh.coerceIn(-24f, 24f))
        postGain[0] = dbToLinear(s.compPostGainLow.coerceIn(-24f, 24f)); postGain[1] = dbToLinear(s.compPostGainLoMid.coerceIn(-24f, 24f)); postGain[2] = dbToLinear(s.compPostGainHiMid.coerceIn(-24f, 24f)); postGain[3] = dbToLinear(s.compPostGainHigh.coerceIn(-24f, 24f))
        attackCoeff[0] = timeCoeff(s.compAttackLow); attackCoeff[1] = timeCoeff(s.compAttackLoMid); attackCoeff[2] = timeCoeff(s.compAttackHiMid); attackCoeff[3] = timeCoeff(s.compAttackHigh)
        releaseCoeff[0] = timeCoeff(s.compReleaseLow); releaseCoeff[1] = timeCoeff(s.compReleaseLoMid); releaseCoeff[2] = timeCoeff(s.compReleaseHiMid); releaseCoeff[3] = timeCoeff(s.compReleaseHigh)
    ...
    private fun applyMbc(buffer: FloatArray, offset: Int, frames: Int) {
        ...
        while (i + 1 < end) {
            val inL = buffer[i]
            val inR = buffer[i + 1]
            val lowL = crossL[0].process(inL); val lowR = crossR[0].process(inR)
            val hp1L = crossL[1].process(inL); val hp1R = crossR[1].process(inR)
            val loMidL = crossL[2].process(hp1L); val loMidR = crossR[2].process(hp1R)
            val hp2L = crossL[3].process(hp1L); val hp2R = crossR[3].process(hp1R)
            val hiMidL = crossL[4].process(hp2L); val hiMidR = crossR[4].process(hp2R)
            val highL = crossL[5].process(hp2L); val highR = crossR[5].process(hp2R)

            val e0 = max(abs(lowL), abs(lowR)).coerceAtLeast(1e-7f) * preGain[0]
            val e1 = max(abs(loMidL), abs(loMidR)).coerceAtLeast(1e-7f) * preGain[1]
            val e2 = max(abs(hiMidL), abs(hiMidR)).coerceAtLeast(1e-7f) * preGain[2]
            val e3 = max(abs(highL), abs(highR)).coerceAtLeast(1e-7f) * preGain[3]
            env[0] = follow(env[0], e0, attackCoeff[0], releaseCoeff[0]); env[1] = follow(env[1], e1, attackCoeff[1], releaseCoeff[1]); env[2] = follow(env[2], e2, attackCoeff[2], releaseCoeff[2]); env[3] = follow(env[3], e3, attackCoeff[3], releaseCoeff[3])
            val target0 = compressionGain(env[0], thresholdDb[0], ratio[0], kneeDb[0]); val target1 = compressionGain(env[1], thresholdDb[1], ratio[1], kneeDb[1]); val target2 = compressionGain(env[2], thresholdDb[2], ratio[2], kneeDb[2]); val target3 = compressionGain(env[3], thresholdDb[3], ratio[3], kneeDb[3])
            compGain[0] = smoothGain(compGain[0], target0, attackCoeff[0], releaseCoeff[0]); compGain[1] = smoothGain(compGain[1], target1, attackCoeff[1], releaseCoeff[1]); compGain[2] = smoothGain(compGain[2], target2, attackCoeff[2], releaseCoeff[2]); compGain[3] = smoothGain(compGain[3], target3, attackCoeff[3], releaseCoeff[3])
            val g0 = compGain[0] * postGain[0]; val g1 = compGain[1] * postGain[1]; val g2 = compGain[2] * postGain[2]; val g3 = compGain[3] * postGain[3]
            buffer[i] = lowL * g0 + loMidL * g1 + hiMidL * g2 + highL * g3
            buffer[i + 1] = lowR * g0 + loMidR * g1 + hiMidR * g2 + highR * g3
            i += 2
        }
    }

    private fun follow(current: Float, input: Float, attack: Float, release: Float): Float = if (input > current) attack * current + (1f - attack) * input else release * current + (1f - release) * input

    private fun compressionGain(level: Float, threshold: Float, ratio: Float, knee: Float): Float {
    ...
    private fun smoothGain(current: Float, target: Float, attack: Float, release: Float): Float = if (target < current) attack * current + (1f - attack) * target else release * current + (1f - release) * target
    ...
    private fun applyLimiter(buffer: FloatArray, offset: Int, frames: Int) {
        ...
        val end = offset + frames * 2
        while (i + 1 < end) {
            val l = buffer[i] * limiterPost
            val r = buffer[i + 1] * limiterPost
            val peak = max(abs(l), abs(r)).coerceAtLeast(1e-7f)
            val overDb = 20f * kotlin.math.log10(peak) - limiterThresholdDb
            val reductionDb = if (overDb > 0f) overDb - overDb / limiterRatio else 0f
            val target = dbToLinear(-reductionDb)
            limiterGain = if (target < limiterGain) limiterAttackCoeff * limiterGain + (1f - limiterAttackCoeff) * target else limiterReleaseCoeff * limiterGain + (1f - limiterReleaseCoeff) * target
            buffer[i] = l * limiterGain
            buffer[i + 1] = r * limiterGain
            i += 2
        }
    }

    private fun timeCoeff(ms: Float): Float = exp(-1.0 / (sr.toDouble() * (ms.coerceIn(0.01f, 2000f) / 1000f))).toFloat()
    private fun dbToLinear(db: Float): Float = 10f.pow(db / 20f)

    private class PcmBiquad(frequency: Float, gainDb: Float, q: Float, type: Type, sampleRate: Float) {
        enum class Type { PEAK, LOW_SHELF, HIGH_SHELF, LOW_PASS, HIGH_PASS, NOTCH, BAND_PASS }
        private var b0 = 1.0; private var b1 = 0.0; private var b2 = 0.0; private var a1 = 0.0; private var a2 = 0.0; private var z1 = 0.0; private var z2 = 0.0
        ...
        fun configure(frequency: Float, gainDb: Float, q: Float, type: Type, sampleRate: Float) {
            val srD = sampleRate.coerceIn(8000f, 192000f).toDouble(); val fc = frequency.coerceIn(1f, sampleRate * 0.49f).toDouble(); val omega = 2.0 * Math.PI * fc / srD
            val sn = kotlin.math.sin(omega); val cs = kotlin.math.cos(omega); val qv = q.coerceIn(0.1f, 40f).toDouble(); val alpha = sn / (2.0 * qv); val A = 10.0.pow(gainDb.coerceIn(-60f, 60f) / 40.0); val sqrtA = sqrt(A); val S = qv.coerceIn(0.1, 10.0); val sa = sn / 2.0 * sqrt((A + 1.0 / A) * (1.0 / S - 1.0) + 2.0); val two = 2.0 * sqrtA * sa
            when (type) {
        ...
        fun process(x: Float): Float { val xd=x.toDouble(); val y=b0*xd+z1; z1=b1*xd-a1*y+z2; z2=b2*xd-a2*y; return y.toFloat() }
    }
}
