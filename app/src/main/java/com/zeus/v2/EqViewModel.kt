...
class EqViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun applyPresetZeusInfrabass() {
        ...
        compMbThHigh = -14f; compMbRatioHigh = 2.5f; compMbKneeHigh = 6f; compMbAttackHigh = 5f; compMbReleaseHigh = 70f; compMbPreGainHigh = 0f; compMbPostGainHigh = 0f
        limiterEnabled = true; limiterThreshold = -1.2f; limiterRatio = 6f; limiterAttack = 1.0f; limiterRelease = 120f; limiterPostGain = 0f
    }

    fun applyPresetBassBoost() {
        applyPresetZeusInfrabass()
        preamp = -4.8f; subBoost = 3f
        if (bands.isNotEmpty()) bands[0] = bands[0].copy(frequency = 40f, gain = 5.5f, q = 0.85f, filterType = EqBand.FilterType.LOW_SHELF)
        if (bands.size > 1) bands[1] = bands[1].copy(frequency = 55f, gain = 2.5f, q = 1.0f)
    }

    fun applyPresetVocalClear() {
        preamp = -3f; subBoost = 0f; bands.clear()
        bands.add(createNewBand(0, 60f).copy(gain = -1.5f, q = 0.9f, filterType = EqBand.FilterType.LOW_SHELF, enabled = true))
        ...
        bands.add(createNewBand(5, 12000f).copy(gain = 1f, q = 0.8f, filterType = EqBand.FilterType.HIGH_SHELF, enabled = true))
        selectedBandIndex = 2; compressorMultibandEnabled = true; limiterEnabled = true; limiterThreshold = -2f; limiterRatio = 8f; limiterAttack = 1f; limiterRelease = 100f
    }

    fun applyPresetJazz() {
        preamp = -4f; subBoost = 0.5f; bands.clear()
        listOf(60f to -1.0f, 120f to 0.5f, 250f to -0.5f, 500f to 0.0f, 1000f to 1.0f, 2200f to 1.5f, 4500f to 1.0f, 8000f to 0.5f, 12000f to 1.0f).forEachIndexed { i, (freq, gain) -> bands.add(createNewBand(i, freq).copy(gain = gain, q = if (freq in 1800f..5000f) 1.15f else 1.0f, enabled = true)) }
        selectedBandIndex = 4; compressorMultibandEnabled = true; limiterEnabled = true; limiterThreshold = -2f; limiterRatio = 5f; limiterAttack = 8f; limiterRelease = 120f
    }

    fun applyPresetBassTreble() {
        preamp = -5f; subBoost = 2f; bands.clear()
        listOf(45f to 3.5f, 100f to 2.5f, 250f to 0f, 500f to -0.5f, 1000f to 0f, 2500f to -0.5f, 5000f to 2.0f, 10000f to 3.0f, 16000f to 3.5f).forEachIndexed { i, (freq, gain) -> val type = when (i) { 0 -> EqBand.FilterType.LOW_SHELF; 8 -> EqBand.FilterType.HIGH_SHELF; else -> EqBand.FilterType.PEAK }; bands.add(createNewBand(i, freq).copy(gain = gain, q = 0.95f, filterType = type, enabled = true)) }
        selectedBandIndex = 0; compressorMultibandEnabled = true; limiterEnabled = true; limiterThreshold = -1.5f; limiterRatio = 6f; limiterAttack = 2f; limiterRelease = 110f
    }

    fun applyPresetAcoustic() {
        preamp = -5.5f; subBoost = 2.5f; bands.clear()
        listOf(32f to 2.8f, 70f to 3.2f, 140f to 1.2f, 280f to -0.8f, 900f to 0.8f, 1800f to 1.8f, 3200f to 2.2f, 6500f to 1.5f, 10000f to 1.8f, 14500f to 1.2f, 18000f to 0.8f).forEachIndexed { i, (freq, gain) -> val type = when { i == 0 -> EqBand.FilterType.LOW_SHELF; i == 10 -> EqBand.FilterType.HIGH_SHELF; else -> EqBand.FilterType.PEAK }; val q = when { freq in 1500f..4000f -> 1.05f; freq >= 6000f -> 0.8f; else -> 1.0f }; bands.add(createNewBand(i, freq).copy(gain = gain, q = q, filterType = type, enabled = true)) }
    ...
    fun toSettings(): EqSettings = EqSettings(
        ...
        compEnabled = compressorMultibandEnabled, cross1 = crossoverFrequencies.getOrElse(0) { 180f }, cross2 = crossoverFrequencies.getOrElse(1) { 1800f }, cross3 = crossoverFrequencies.getOrElse(2) { 8000f }, cross4 = crossoverFrequencies.getOrElse(3) { 20000f }, cross4 = crossoverFrequencies.getOrElse(3) { 20000f }, cross4 = crossoverFrequencies.getOrElse(3) { 20000f }, cross4 = crossoverFrequencies.getOrElse(3) { 20000f }, cross4 = crossoverFrequencies.getOrElse(3) { 20000f }, cross4 = crossoverFrequencies.getOrElse(3) { 20000f }, cross4 = crossoverFrequencies.getOrElse(3) { 20000f }, cross4 = crossoverFrequencies.getOrElse(3) { 20000f }, cross4 = crossoverFrequencies.getOrElse(3) { 20000f }, cross4 = crossoverFrequencies.getOrElse(3) { 20000f },
        ...
        compAttackLow = compMbAttackLow, compAttackLoMid = compMbAttackLoMid, compAttackHiMid = compMbAttackHiMid, compAttackHigh = compMbAttackHigh,
        compReleaseLow = compMbReleaseLow, compReleaseLoMid = compMbReleaseLoMid, compReleaseHiMid = compMbReleaseHiMid, compReleaseHigh = compMbReleaseHigh,
    ...
    fun saveNamedPreset(name: String) { val clean = name.trim(); if (clean.isNotEmpty()) { val settings=toSettings(); EqPrefs.saveNamed(getApplication(), clean, settings); ConfigFileRepository.saveNamed(getApplication(), clean, settings) } }
    fun loadNamedPreset(name: String) { EqPrefs.loadNamed(getApplication(), name)?.let { loadFrom(it) } }
    ...
    fun selectTarget(context: android.content.Context, target: TargetModel) { selectedTargetName = target.name; targetCurve = AutoEqRepository.loadTarget(context, target) }
    fun applyAutoEqProfile(profile: AutoEqProfile) { preamp = profile.preamp; bands.clear(); profile.filters.take(MAX_BANDS).forEachIndexed { i, f -> bands.add(createNewBand(i, f.frequency).copy(gain=f.gain, q=f.q, filterType=f.type, enabled=true)) }; if (bands.isEmpty()) bands.add(createNewBand(0, 1000f)); selectedBandIndex = 0; limiterEnabled = true }
    fun saveSettings() { val settings=toSettings(); EqPrefs.save(getApplication(), settings); ConfigFileRepository.saveLatest(getApplication(), settings) }
    fun loadSavedIfAny() { EqPrefs.load(getApplication())?.let { loadFrom(it) } }
    fun selectBand(index: Int) { if (index in bands.indices) selectedBandIndex = index }
    fun updateSelectedBand(frequency: Float? = null, gain: Float? = null, q: Float? = null, enabled: Boolean? = null, filterType: EqBand.FilterType? = null) { val idx = selectedBandIndex; if (idx !in bands.indices) return; val current = bands[idx]; bands[idx] = current.copy(frequency = frequency?.coerceIn(1f, 30000f) ?: current.frequency, gain = gain?.coerceIn(-30f, 30f) ?: current.gain, q = q?.coerceIn(0.1f, 40f) ?: current.q, enabled = enabled ?: current.enabled, filterType = filterType ?: current.filterType) }
    fun addBand() { if (bands.size >= MAX_BANDS) return; val newId = (bands.maxOfOrNull { it.id } ?: -1) + 1; val newFreq = if (bands.isEmpty()) 1000f else (bands.last().frequency * 1.8f).coerceIn(1f, 30000f); bands.add(createNewBand(newId, newFreq)); selectedBandIndex = bands.lastIndex }
    fun removeSelectedBand() { if (bands.size <= 1) return; val idx = selectedBandIndex; if (idx !in bands.indices) return; bands.removeAt(idx); selectedBandIndex = idx.coerceIn(0, bands.lastIndex) }
    fun nextSection() { currentSection = when (currentSection) { EqSection.EQUALIZER -> EqSection.CROSSOVER; EqSection.CROSSOVER -> EqSection.LIMITER; EqSection.LIMITER -> EqSection.AUTOEQ; EqSection.AUTOEQ -> EqSection.EQUALIZER; EqSection.PIPELINE -> EqSection.EQUALIZER } }
    fun previousSection() { currentSection = when (currentSection) { EqSection.EQUALIZER -> EqSection.AUTOEQ; EqSection.CROSSOVER -> EqSection.EQUALIZER; EqSection.LIMITER -> EqSection.CROSSOVER; EqSection.AUTOEQ -> EqSection.LIMITER; EqSection.PIPELINE -> EqSection.LIMITER } }
    fun sectionTitle(): String = when (currentSection) { EqSection.EQUALIZER -> "Equalizer"; EqSection.CROSSOVER -> "Compresor Multibanda"; EqSection.LIMITER -> "Limiter"; EqSection.AUTOEQ -> "AutoEQ / Presets"; EqSection.PIPELINE -> "Equalizer" }
