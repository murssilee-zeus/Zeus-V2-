...
data class EqBand(
    ...
    fun copyValuesFrom(other: EqBand) {
        frequency = other.frequency.coerceIn(1f, 30000f); gain = other.gain.coerceIn(-30f, 30f); q = other.q.coerceIn(0.1f, 40f); enabled = other.enabled; filterType = other.filterType
    }
}
...
fun createDefaultBands(): List<EqBand> {
    val defaults = listOf(Triple(31f,5f,.7f),Triple(62f,4f,1f),Triple(125f,2.5f,1.2f),Triple(250f,1f,1f),Triple(500f,0f,1f),Triple(1000f,0f,1f),Triple(2000f,0f,1.2f),Triple(4000f,0f,1.5f),Triple(8000f,0f,1f),Triple(16000f,-1f,.8f))
    val initial=ArrayList<EqBand>(16)
    for(i in 0 until 16){val (f,g,q)=if(i<defaults.size) defaults[i] else Triple(1000f+(i-10)*1500f,0f,1f);initial+=EqBand(i,f,g,q,true,when(i){0->EqBand.FilterType.LOW_SHELF;15->EqBand.FilterType.HIGH_SHELF;else->EqBand.FilterType.PEAK},BAND_COLORS[i%BAND_COLORS.size])}
    return initial
}
